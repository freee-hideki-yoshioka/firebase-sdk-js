...
class FreeeCryptor {
    ...
    async decrypt(token) {
        ...
        const decryptAccessToken = this.crypt(accessToken, this.decipher(algorithm, key, iv), OUT, IN);
        if (this.accessToken === decryptAccessToken) {
            console.log("access token decrypt success");
        }
        const decryptRefreshToken = this.crypt(refreshToken, this.decipher(algorithm, key, iv), OUT, IN);
        if (this.refreshToken === decryptRefreshToken) {
            console.log("refreshToken decrypt success");
        }
        return Object.assign(Object.assign({}, token), { accessToken: decryptAccessToken, refreshToken: decryptRefreshToken });
    }
    cipher(cryptoKey, iv) {
